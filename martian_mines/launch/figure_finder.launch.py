...
def generate_launch_description():
    ...
    uav0_nodes = [
        ...
        Node(
            package='martian_mines',
            executable='figure_finder.py',
            name='figure_finder',
            output='screen'
        ),
        Node(
            package='martian_mines',
            executable='report_uploader.py',
            name='uploader',
            output='screen'
        ),
        # Execute ROS 2 service call
        ExecuteProcess(
            cmd=['ros2', 'service', 'call', '/figure_finder/start', 'std_srvs/srv/Trigger'],
            name='figure_finder_start',
            output='screen'
        )
    ]

    # Handle remapping if real_world mode is enabled
    remap_real_world = [
        Node(
            package='tf2_ros',
    ...
    return LaunchDescription([
        ...
        no_start_pose_arg,
        core_launch
    ] + uav0_nodes + (remap_real_world if LaunchConfiguration('real_world') else []))
