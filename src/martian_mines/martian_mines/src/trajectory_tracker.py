...
class TrajectoryTracker(Node):
    def __init__(self):
        ...
        self.velocity = (
            self.get_parameter("velocity").get_parameter_value().integer_value
        )
        ...
        self.pub_finished = self.create_publisher(
            Empty, "trajectory_tracker/finished", 1
        )
        self.sub_trajectory = self.create_subscription(
            Path, "trajectory_tracker/path", self.callback_trajectory, 10
        )
